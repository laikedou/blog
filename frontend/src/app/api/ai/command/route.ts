      ...
      execute: async ({ writer }) => {
        ...
        if (!toolName) {
          ...
          const enumOptions = isSelecting
            ...
            : ['generate', 'comment'];
          const modelId = model || DEFAULT_TOOL_MODEL;
        ...
        const stream = streamText({
          ...
          model: getModel(model || DEFAULT_MODEL, gatewayApiKey),
          // Not used
          prompt: '',
          tools: {
            comment: getCommentTool(editor, {
              messagesRaw,
              model: getModel(model || DEFAULT_TOOL_MODEL, gatewayApiKey),
              writer,
            }),
          ...
          prepareStep: async (step) => {
            ...
            if (toolName === 'edit') {
              ...
              return {
                ...
                model:
                  editType === 'selection'
                    ? getModel(model || DEFAULT_TOOL_MODEL, gatewayApiKey)
                    : getModel(model || DEFAULT_MODEL, gatewayApiKey),
                messages: [
                  {
                    content: editPrompt,
                    role: 'user',

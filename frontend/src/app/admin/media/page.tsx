...
export default function AdminMediaPage() {
  ...
  const handleDelete = async (id: number) => {
    const ok = await confirm({ title: t('admin.deleteFile'), message: t('admin.confirmDeleteFile'), confirmLabel: t('common.delete'), variant: 'destructive' });
    if (!ok) return;
    await mediaApi.delete(id); fetchMedia();
  };

  const copyUrl = async (url: string) => { await navigator.clipboard.writeText(url); toast.success(t('admin.urlCopied')); };

  return (
    <div>
      <div className="flex items-center justify-between mb-8">
        <h1 className="font-display text-display-md text-ink">{t('admin.media')}</h1>
        <label className="cursor-pointer">
          <Button disabled={uploading}><Upload className="h-4 w-4 mr-2" />{uploading ? t('admin.uploading') : t('common.upload')}</Button>
      ...
      {loading ? (
        ...
        <Card><CardContent className="p-12 text-center"><p className="text-body text-ink-muted mb-1">{t('admin.noMediaYet')}</p><p className="text-body-sm text-ink-muted">{t('admin.mediaUploadHint')}</p></CardContent></Card>
      ) : (
        <>
          <div className="grid grid-cols-2 md:grid-cols-4 lg:grid-cols-6 gap-4">
            {media.map((item: any) => (
              <Card key={item.id} className="overflow-hidden group shadow-card">
                <div className="h-32 bg-cream-200 relative overflow-hidden">
                  {item.mimeType?.startsWith('image/') ? (
                    <img src={item.url} alt={item.originalName} className="w-full h-full object-cover" loading="lazy" onError={(e) => { (e.target as HTMLImageElement).src = `data:image/svg+xml,<svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 100 100"><rect fill="%23f0ebe3" width="100" height="100"/><text fill="%238a8478" font-size="10" x="50" y="55" text-anchor="middle">${t('admin.brokenImage')}</text></svg>`; }} />
                  ) : (
                  ...
                  <div className="absolute inset-0 bg-ink/60 opacity-0 group-hover:opacity-100 transition-opacity flex items-center justify-center gap-2">
                    <Button variant="secondary" size="sm" className="h-8" onClick={() => copyUrl(item.url)}><Copy className="h-3 w-3 mr-1" />{t('admin.url')}</Button>
                    <Button variant="destructive" size="sm" className="h-8" onClick={() => handleDelete(item.id)}><Trash2 className="h-3 w-3 mr-1" />{t('common.delete')}</Button>
                  </div>
                </div>
                <CardContent className="p-3">
                  <p className="text-body-sm text-ink truncate">{item.originalName}</p>
                  <p className="text-caption-sm text-ink-muted">{(item.size / 1024).toFixed(1)} {t('admin.kb')}</p>
                </CardContent>
              </Card>
            ))}
          </div>
